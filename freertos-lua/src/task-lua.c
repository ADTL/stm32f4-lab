...
void stack_dump (lua_State *L)
{
	...
	while (lua_gettop(L) > 0)
	{
		...
		switch (t)
		{
			...
			sprintf(buffer, "`%s'", lua_tostring(L, 1));
			break;

			case LUA_TBOOLEAN:  /* booleans */
			sprintf(buffer, lua_toboolean(L, 1) ? "true" : "false");
			break;
			...
			sprintf(buffer, "%s", lua_typename(L, 1));
			break;
		}
...
void tasklua_init(xQueueHandle *p_queue_lua_commands)
{
