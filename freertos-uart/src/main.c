...
#define MAX_STRLEN 40 // this is the maximum string length of our string in characters
...
#define BLUE_COUNTER_VAL 2

volatile uint8_t blue_counter = 0; // will be used every time UART receives a char
...
void USART_char(USART_TypeDef* USARTx, volatile char s)
{
	// wait until data register is empty
	while( !(USARTx->SR & 0x00000040) )
		;
...
void task_led_status(void* p)
{
    while(1)
    {
    	...
    	// se o contador de blue estiver maior que BLUE_COUNTER_VAL, poe em erro!
    	if (blue_counter > (BLUE_COUNTER_VAL * 10) )
    	{
    		led_state = LED_STATUS_RED;
    	}
    	...
    	//    	GPIO_ToggleBits(GPIOD, GPIO_LED_GREEN | GPIO_LED_ORANGE | GPIO_LED_RED | GPIO_LED_BLUE);
    	vTaskDelay(150);
    }
}
