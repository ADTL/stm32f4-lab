...
{
  uint8_t dataWidth = FSMC_Bank1->BTCR[FSMC_Bank1_NORSRAM2];
  if(dataWidth & FSMC_MemoryDataWidth_8b)
...
void SRAM_WriteBuffer_8(uint8_t* pBuffer, uint32_t WriteAddr, uint32_t NumHalfwordToWrite)
{
  ...
  for (; NumHalfwordToWrite != 0; NumHalfwordToWrite--) /* while there is data to write */
  {
    bufferWord = (uint16_t)*pBuffer;
    bufferWord = bufferWord << 8;
    *pBuffer++;
    bufferWord |= (uint16_t)*pBuffer;
    *pBuffer++;
    
    /* Transfer data to the memory */
    *(uint16_t *) (Bank1_SRAM2_ADDR + WriteAddr) = bufferWord;
...
void SRAM_ReadBuffer_8(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumHalfwordToRead)
{
  ...
  {
    /* Read a half-word from the memory */
    bufferWord = *(__IO uint16_t*) (Bank1_SRAM2_ADDR + ReadAddr);
    
    *pBuffer = (uint8_t) (bufferWord >> 8);
    *pBuffer++;
    *pBuffer = (uint8_t) bufferWord;
    *pBuffer++;
    /* Increment the address*/
    ReadAddr += 2;
  }
